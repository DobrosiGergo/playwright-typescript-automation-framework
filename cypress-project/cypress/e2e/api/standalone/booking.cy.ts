  ...
  describe('Positive Test Cases @smoke', () => {
    ...
    it('should create a new booking and verify details', () => {
      ...
      bookingService.createBooking(uniqueBooking).then((createResponse) => {
        ...
        softExpect(createResponse.body.booking.firstname).to.eq(uniqueBooking.firstname);
        softExpect(createResponse.body.booking.lastname).to.eq(uniqueBooking.lastname);
        softExpect(createResponse.body.booking.totalprice).to.eq(uniqueBooking.totalprice);
        softExpect(createResponse.body.booking.depositpaid).to.eq(uniqueBooking.depositpaid);
    ...
    it('should retrieve all booking IDs', () => {
      bookingService.getAllBookingIds().then((response) => {
        ...
        expect(response.body.length).to.be.greaterThan(BOOKING_VALIDATION.MIN_BOOKING_IDS_COUNT);
      });
    });

    it('should filter bookings by name using shared booking', () => {
      bookingService.getAllBookingIds({ firstname: sharedBooking.firstname }).then((response) => {
        softExpect(response.status).to.eq(StatusCodes.OK);
        softExpect(response.body).to.be.an('array');
        ...
        authService.createToken(defaultCredentials).then((authResponse) => {
          ...
          bookingService
            ...
            .then((updateResponse) => {
              ...
              softExpect(updateResponse.body.totalprice).to.eq(updatedBooking.totalprice);
              softExpect(updateResponse.body.additionalneeds).to.eq(updatedBooking.additionalneeds);

              bookingService.getBookingById(createdBookingId).then((getResponse) => {
                softExpect(getResponse.body.totalprice).to.eq(updatedBooking.totalprice);
                expect(getResponse.body.additionalneeds).to.eq(updatedBooking.additionalneeds);
              });
            });
        });
      });
      ...
      bookingService.createBooking(uniqueBooking).then((createResponse) => {
        ...
        authService.createToken(defaultCredentials).then((authResponse) => {
          ...
          bookingService
            ...
            .then((updateResponse) => {
              softExpect(updateResponse.status).to.eq(StatusCodes.OK);
              softExpect(updateResponse.body.firstname).to.eq(partialUpdate.firstname);
              softExpect(updateResponse.body.lastname).to.eq(partialUpdate.lastname);
              expect(updateResponse.body.totalprice).to.eq(uniqueBooking.totalprice);
            });
        });
      });
    });
    ...
    it('should delete a booking and verify deletion', () => {
      ...
      bookingService.createBooking(uniqueBooking).then((createResponse) => {
        ...
        authService.createToken(defaultCredentials).then((authResponse) => {
          ...
          bookingService.deleteBooking(createdBookingId, token).then((deleteResponse) => {
  ...
  describe('Negative Test Cases @negative', () => {
    it('should reject update without authentication token', () => {
      ...
      bookingService.createBooking(uniqueBooking).then((createResponse) => {
        ...
        bookingService.updateBooking(createdBookingId, updatedBooking).then((response) => {
